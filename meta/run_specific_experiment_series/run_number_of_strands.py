...
def main():
    ...
    # Get all number_of_strands series, ordered by name
    series_list = session.query(ExperimentSeries).filter(
        ExperimentSeries.group_name == 'number_of_strands'
    ).order_by(ExperimentSeries.experiment_series_name).all()

    print(f"Found {len(series_list)} number_of_strands experiment series to run\n")
    ...
    session.close()
    print(f"\n✅ All {len(series_list)} number_of_strands series completed!")
